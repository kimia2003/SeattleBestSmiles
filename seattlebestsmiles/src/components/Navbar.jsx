...
const LinkItem = ({ to, children, icon, extraClass = "" }) => (
  <li className={`relative px-4 py-2 flex items-center space-x-2 rounded-md transition-all ${extraClass}`}>
    {icon && <FaPhone className="text-green-400" />}
    <Link to={to} className="text-white hover:border-b-2 hover:border-white transition-all">
      {children}
    </Link>
  </li>
);
          ...
          <ul className="flex flex-wrap justify-start items-center space-x-4 w-full md:w-auto">
            ...
            <li className="relative p-4">
              <button
                ...
                className="focus:outline-none hover:border-b-2 hover:border-white transition-all"
              >
                About
              </button>
              {dropdownOpen && (
                <ul ref={dropdownRef} className="absolute left-0 mt-2 w-40 bg-white text-black shadow-lg">
